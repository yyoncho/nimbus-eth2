...
QtObject:
  ...
  proc newMainModel*(app: QApplication, url: string): MainModel =
    let
      client = RestClientRef.new(url).get()

    var
      headBlock = (waitFor client.getBlockV2(BlockIdent.init(BlockIdentType.Head), defaultRuntimeConfig)).get()
  ...
  proc onLoadBlock(self: MainModel, root: string) {.slot.} =
    try:
      var blck = waitFor(self.client.getBlockV2(
        BlockIdent.decodeString(root).tryGet(), defaultRuntimeConfig))
      if blck.isSome():
