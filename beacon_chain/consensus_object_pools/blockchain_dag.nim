...
proc getBlock*(
  ...
  dag.db.getBlock(bid.root, T) or
    getBlock(
      dag.era, getStateField(dag.headState.data, historical_roots).asSeq,
...
proc getForkedBlock*(
  ...
  withBlck(result.get()):
    ...
    blck = getBlock(dag, bid, T).valueOr:
        getBlock(
            dag.era, getStateField(dag.headState.data, historicalRoots).asSeq,
...
proc init*(T: type ChainDAGRef, cfg: RuntimeConfig, db: BeaconChainDB,
  ...
  dag.era = EraDB.new(
    cfg, eraPath, getStateField(dag.headState.data, genesis_validators_root))
  ...
  if dag.backfill.slot > 0: # See if we can frontfill blocks from era files
    ...
    let
      historical_roots = getStateField(
        dag.headState.data, historical_roots).asSeq()
  ...
  info "Block DAG initialized",
    ...
    frontfill = (dag.frontfill.slot, shortLog(dag.frontfill.root)),
